...
void sub_division_tri(int level,fvec4_t* buffer, int* start_index, const fvec4_t* p1, const fvec4_t* p2, const fvec4_t* p3)
{
	...
	{
		fvec4_t p1p2 = add_v4v4(p1 , p2);
		p1p2 = normalize_v4(&p1p2);

		fvec4_t p2p3 = add_v4v4(p2, p3);
		p2p3 = normalize_v4(&p2p3);

		fvec4_t p3p1 = add_v4v4(p3, p1);
...
void gen_sphere(int level,const fvec4_t* src_buffer,const int src_size,fvec4_t* dst_buffer,int* dst_size)
{
	*dst_size = 0;
	...
	{
		fvec4_t p1 = src_buffer[i];
		p1 = normalize_v4(&p1);
		fvec4_t p2 = src_buffer[i+1];
		p2 = normalize_v4(&p2);
		fvec4_t p3 = src_buffer[i+2];
		p3 = normalize_v4(&p3);
